package com.wangbao.springbootrocketmqservice.service.impl;
...
public class RocketMqProducer {
    ...
    public static RocketMqProducer rocketMqProducer;
    @PostConstruct
    public void init()
    {
        rocketMqProducer=this;
    }
    ...
    public <T> void send(String topic, T msg) {
        rocketMqProducer.rocketMQTemplate.convertAndSend(topic + ":tag1", msg);
    ...
    public <T> SendResult sendTagMsg(String topic, String tag, T msg) {
        topic = topic + ":" + tag;
        return rocketMqProducer.rocketMQTemplate.syncSend(topic, MessageBuilder.withPayload(msg).build());
    }
    ...
    public <T> SendResult sendMsg(String topic, T msg) {
        ...
        SendResult sendResult = rocketMqProducer.rocketMQTemplate.syncSend(topic, message);
    ...
    public void asyncSend(String topic, Message<?> message, SendCallback sendCallback) {
        rocketMqProducer.rocketMQTemplate.asyncSend(topic, message, sendCallback);
    }
    ...
    public void asyncSend(String topic, Message<?> message, SendCallback sendCallback, long timeout) {
        rocketMqProducer.rocketMQTemplate.asyncSend(topic, message, sendCallback, timeout);
    }
    ...
    public <T> void sendDelay(String topic, T msg, long timeout, int delayLevel) {
        Message<T> message = MessageBuilder.withPayload(msg).build();
        rocketMqProducer.rocketMQTemplate.syncSend(topic, message, timeout, delayLevel);
    }
    ...
    public void asyncSendDelay(String topic, Message<?> message, SendCallback sendCallback, long timeout, int delayLevel) {
        rocketMqProducer.rocketMQTemplate.asyncSend(topic, message, sendCallback, timeout, delayLevel);
    }
    ...
    public void asyncSendDelay(String topic, Message<?> message, long timeout, int delayLevel) {
        rocketMqProducer.rocketMQTemplate.asyncSend(topic, message, new SendCallback() {
            @Override
            public void onSuccess(SendResult sendResult) {
                log.info("topic:{}消息---发送MQ成功---", topic);
            }
    ...
    public <T> void sendOneWayMsg(String topic, T msg) {
        Message<T> message = MessageBuilder.withPayload(msg).build();
        rocketMqProducer.rocketMQTemplate.sendOneWay(topic, message);
    }
    ...
    public <T> void syncSendOrderly(String topic, T msg, String hashKey) {
        ...
        rocketMqProducer.rocketMQTemplate.syncSendOrderly(topic, message, hashKey);
    }
    ...
    public <T> void syncSendOrderly(String topic, T msg, String hashKey, long timeout) {
        ...
        rocketMqProducer.rocketMQTemplate.syncSendOrderly(topic, message, hashKey, timeout);
    }
    ...
    public <T> SendResult asyncSendBatch(String topic, List<T> msgList) {
        ...
        return rocketMqProducer.rocketMQTemplate.syncSend(topic, messageList);
    }
    ...
    public <T> void sendTransaction(String txProducerGroup, String topic, String tag, T msg, T arg){
        ...
        Message<T> message = MessageBuilder.withPayload(msg)
                ...
                .build();
        TransactionSendResult result = rocketMqProducer.rocketMQTemplate.sendMessageInTransaction(txProducerGroup, topic, message, arg);
